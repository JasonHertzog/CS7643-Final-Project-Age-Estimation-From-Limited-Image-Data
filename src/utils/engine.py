...
def train(model, dataloader, optimizer, criterion, scheduler=None, device='cpu'):
    ...
    for batch_idx, (images, labels) in enumerate(progress_bar):
        ...
        targets = labels['age'].to(device)          # (B,)import torch
import torch.nn as nn
from tqdm import tqdm
...
def train(model, dataloader, optimizer, criterion, task_type='regression', target_col='age', scheduler=None, device='cpu'):
    """Train for one epoch on UTKFace age estimation."""
    model.train()
    ...
    for batch_idx, (images, labels) in enumerate(progress_bar):
        images = images.to(device)
        targets = labels[target_col].to(device) # grab correct target column
